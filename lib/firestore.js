...
export async function getAllStudents() {
  ...
  const snapshot = await getDocs(q);
  // Filter to only show approved students or students without status (legacy data)
  return snapshot.docs
    .map(doc => ({ id: doc.id, ...doc.data() }))
    .filter(student => !student.status || student.status === 'approved');
}
...
export async function registerStudent(studentData) {
  ...
  if (studentData.email) {
    const existingEmail = await getStudentByEmail(studentData.email);
    if (existingEmail) {
...
export async function approveStudent(docId) {
  const docRef = doc(db, 'students', docId);
  await updateDoc(docRef, {
    status: 'approved',
...
export async function rejectStudent(docId) {
  const docRef = doc(db, 'students', docId);
  await deleteDoc(docRef);
}

export async function getPendingStudentsCount() {
